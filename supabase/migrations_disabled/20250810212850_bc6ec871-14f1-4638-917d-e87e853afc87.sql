-- Remove SECURITY DEFINER from functions to satisfy linter and follow least-privilege
...
DECLARE evt_type TEXT; mod TEXT; BEGIN
  ...
  RETURN NEW;
END; $$ LANGUAGE plpgsql; -- default SECURITY INVOKER
...
-- Ensure event_bus has at least one permissive policy (already exists, but re-assert idempotently)
DO $$ BEGIN
  IF NOT EXISTS (
