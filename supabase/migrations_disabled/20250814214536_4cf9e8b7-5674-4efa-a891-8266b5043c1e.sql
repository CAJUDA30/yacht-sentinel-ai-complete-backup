...
-- Add new columns to existing audit_instances table
ALTER TABLE public.audit_instances 
ADD COLUMN IF NOT EXISTS category_id UUID REFERENCES public.claims_repairs_categories(id),
ADD COLUMN IF NOT EXISTS supplier_contractor_id UUID REFERENCES public.suppliers_contractors(id),
ADD COLUMN IF NOT EXISTS job_type_specific TEXT DEFAULT 'general' CHECK (job_type_specific IN ('warranty_claim', 'insurance_claim', 'maintenance_repair', 'emergency_repair', 'preventive_maintenance', 'general')),
...
ADD COLUMN IF NOT EXISTS communication_preference TEXT DEFAULT 'email' CHECK (communication_preference IN ('email', 'whatsapp', 'both')),
...
-- Update existing cost_estimates table to add missing columns if they don't exist
DO $$ 
BEGIN
  -- Add supplier_contractor_id if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'cost_estimates' 
                 AND column_name = 'supplier_contractor_id') THEN
    ALTER TABLE public.cost_estimates 
    ADD COLUMN supplier_contractor_id UUID REFERENCES public.suppliers_contractors(id);
  END IF;

  -- Add communication_thread if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'cost_estimates' 
                 AND column_name = 'communication_thread') THEN
    ALTER TABLE public.cost_estimates 
    ADD COLUMN communication_thread JSONB DEFAULT '[]'::jsonb;
  END IF;
...
CREATE INDEX IF NOT EXISTS idx_suppliers_contractors_type ON public.suppliers_contractors(type);
CREATE INDEX IF NOT EXISTS idx_suppliers_contractors_category ON public.suppliers_contractors(category);
CREATE INDEX IF NOT EXISTS idx_suppliers_contractors_status ON public.suppliers_contractors(status);
CREATE INDEX IF NOT EXISTS idx_suppliers_contractors_rating ON public.suppliers_contractors(overall_rating);

CREATE INDEX IF NOT EXISTS idx_supplier_assignments_module ON public.supplier_module_assignments(module_name);
CREATE INDEX IF NOT EXISTS idx_supplier_assignments_preferred ON public.supplier_module_assignments(preferred) WHERE preferred = true;

CREATE INDEX IF NOT EXISTS idx_categories_type ON public.claims_repairs_categories(type);
CREATE INDEX IF NOT EXISTS idx_categories_parent ON public.claims_repairs_categories(parent_id);
CREATE INDEX IF NOT EXISTS idx_categories_active ON public.claims_repairs_categories(is_active) WHERE is_active = true;

CREATE INDEX IF NOT EXISTS idx_audit_instances_category ON public.audit_instances(category_id);
CREATE INDEX IF NOT EXISTS idx_audit_instances_supplier ON public.audit_instances(supplier_contractor_id);
CREATE INDEX IF NOT EXISTS idx_audit_instances_job_type ON public.audit_instances(job_type_specific);
CREATE INDEX IF NOT EXISTS idx_audit_instances_urgency ON public.audit_instances(urgency_level);
