...
-- Create scan events table for tracking all scans
CREATE TABLE public.scan_events (
  ...
  created_inventory_item BOOLEAN DEFAULT false,
  created_equipment BOOLEAN DEFAULT false,
...
-- Create RLS policies for scan_events
CREATE POLICY "Users can view their scan events" 
...
CREATE POLICY "Users can update their scan events" 
...
CREATE INDEX idx_scan_events_session ON public.scan_events(session_id);
CREATE INDEX idx_scan_events_user ON public.scan_events(user_id);
CREATE INDEX idx_scan_events_product ON public.scan_events(master_product_id);