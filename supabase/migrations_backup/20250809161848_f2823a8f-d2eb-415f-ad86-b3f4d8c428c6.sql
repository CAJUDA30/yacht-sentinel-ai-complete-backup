-- Create new tables and columns for Multi-AI Configuration & Orchestration

-- 1) Extend ai_providers with api_secret_name reference
ALTER TABLE public.ai_providers
ADD COLUMN IF NOT EXISTS api_secret_name text;

-- 2) ai_configs: active model config per module/provider
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_ai_configs_updated_at'
  ) THEN
...
-- 3) module_mappings: per-module provider/model mapping and defaults
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_module_mappings_updated_at'
  ) THEN
...
-- 4) ai_field_rules: per-field enable/validation/human review
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_ai_field_rules_updated_at'
  ) THEN
...
-- 5) ai_provider_logs: connection/tests and general provider logs
...
-- 6) ai_health: provider health snapshot
...
-- 7) ai_usage_metrics: aggregated usage
...
CREATE INDEX IF NOT EXISTS idx_ai_usage_metrics_window ON public.ai_usage_metrics(window);
CREATE INDEX IF NOT EXISTS idx_ai_usage_metrics_collected ON public.ai_usage_metrics(collected_at);

-- 8) llm_provider_models: cached model list per provider
...
-- 9) ai_action_logs: auditing of AI actions
...
-- 10) ai_jobs & ai_job_attempts: lightweight job queue tables
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trg_ai_jobs_updated_at'
  ) THEN
