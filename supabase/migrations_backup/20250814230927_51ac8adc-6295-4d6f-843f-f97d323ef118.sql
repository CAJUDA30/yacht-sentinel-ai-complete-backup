...
CREATE TABLE public.crew_schedules (
  ...
  efficiency_score NUMERIC(3,2) DEFAULT 0.85 CHECK (efficiency_score >= 0 AND efficiency_score <= 1),
...
CREATE TABLE public.equipment_health_metrics (
  ...
  overall_health_score NUMERIC(3,2) NOT NULL CHECK (overall_health_score >= 0 AND overall_health_score <= 100),
