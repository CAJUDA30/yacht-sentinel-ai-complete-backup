...
COMMENT ON POLICY "Authenticated delete access" ON "public"."system_settings" IS 'REDUNDANT: This policy duplicates Superadmin full access DELETE. Consider removing in future cleanup.';
...
CREATE POLICY "Superadmin full access" ON "public"."inventory_items" TO "authenticated" USING (("auth"."uid"() IN ( SELECT "users"."id"
   FROM "auth"."users"
  WHERE (("users"."email")::"text" = 'superadmin@yachtexcel.com'::"text")))) WITH CHECK (("auth"."uid"() IN ( SELECT "users"."id"
   FROM "auth"."users"
  WHERE (("users"."email")::"text" = 'superadmin@yachtexcel.com'::"text"))));
