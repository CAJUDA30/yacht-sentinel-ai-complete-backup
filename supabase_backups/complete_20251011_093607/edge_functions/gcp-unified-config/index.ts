...
async function handleRunTest(payload: any) {
  ...
  try {
    ...
    if (payload?.documentDataUrl || payload?.documentBase64 || payload?.docB64) {
      // Check if we have credentials - if not, return mock data for development
      const hasCredentials = Boolean(Deno.env.get("GOOGLE_SERVICE_ACCOUNT_JSON"));
      
      if (!hasCredentials) {
        console.log(`🚨 [gcp-unified-config] MISSING GOOGLE CLOUD CREDENTIALS`);
        console.log(`📋 To enable real Google Cloud Document AI processing:`);
        console.log(`   1. Add your GOOGLE_SERVICE_ACCOUNT_JSON to supabase/functions/.env`);
        console.log(`   2. Set GOOGLE_CLOUD_PROJECT_ID=yachtexcel1`);
        console.log(`   3. Set DOCUMENT_AI_PROCESSOR_ID=8708cd1d9cd87cc1`);
        console.log(`⚠️  Currently returning MOCK data for development...`);
        ...
        const mockFields = {
          ...
          gross_tonnage: "120",
          home_port: "George Town"
        };
        
        // Mock Document AI response structure
        const mockDocumentAI = {
          document: {
            text: "CERTIFICATE OF REGISTRY\n\nYacht Name: Ocean Explorer\nRegistration: CI-2018-001\nFlag State: Cayman Islands\nLength: 45.0m\nBeam: 8.5m\nYear Built: 2018\nBuilder: Sunseeker\nOwner: Super Admin",
            entities: Object.entries(mockFields).map(([key, value]) => ({
              type: key,
              mentionText: value,
              confidence: 0.95 + Math.random() * 0.04, // Random confidence between 0.95-0.99
              textAnchor: {
                textSegments: [{ startIndex: 0, endIndex: value.length }]
              }
            }))
          }
        };
        ...
        out.extractedFields = mockFields;
        out.confidence = 0.97;
        out.steps.push({ provider: 'documentAI-mock', processorId: 'mock-development' });
        
        const total = Date.now() - started;
        await recordLog({ action: 'run_test', provider: 'documentAI-mock', success: true, latency_ms: total, details: { steps: out.steps.length, mock: true } });
        return { ...out, metrics: { total_ms: total, processor_id: 'mock-development' } };
      }
