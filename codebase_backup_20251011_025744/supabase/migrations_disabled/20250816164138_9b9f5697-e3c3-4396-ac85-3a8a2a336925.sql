...
-- 3) Backfill last 14 days into analytics_events for visibility (idempotent per migration run)
-- Map key AI logs into analytics_events with source metadata
WITH cutoff AS (
  SELECT now() - interval '14 days' AS ts
)
...
SELECT 
  ...
  NULL
FROM public.ai_model_logs aml, cutoff c
WHERE aml.created_at >= c.ts
...
SELECT 
  ...
  COALESCE(aml.error_message, 'performance_log') AS event_message,
  'ai' AS module,
  ...
  NULL
FROM public.ai_performance_logs apl, cutoff c
WHERE apl.created_at >= c.ts
ON CONFLICT DO NOTHING;
...
SELECT 
  ...
  NULL
FROM public.ai_processing_logs apl, cutoff c
WHERE apl.created_at >= c.ts
ON CONFLICT DO NOTHING;