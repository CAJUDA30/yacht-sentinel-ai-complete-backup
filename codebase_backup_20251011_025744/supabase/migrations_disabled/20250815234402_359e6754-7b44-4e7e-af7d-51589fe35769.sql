-- Phase 3: Proactive Intelligence & Smart Knowledge Database Tables (Fixed)
...
-- Smart knowledge items table (using JSONB for embeddings)
...
CREATE INDEX IF NOT EXISTS idx_user_actions_user_module ON public.user_actions(user_id, module);
CREATE INDEX IF NOT EXISTS idx_user_actions_created_at ON public.user_actions(created_at);
CREATE INDEX IF NOT EXISTS idx_behavior_patterns_user_module ON public.behavior_patterns(user_id, module);
CREATE INDEX IF NOT EXISTS idx_proactive_suggestions_user_active ON public.proactive_suggestions(user_id, dismissed, acted_upon);
CREATE INDEX IF NOT EXISTS idx_smart_knowledge_module ON public.smart_knowledge_items(module);
CREATE INDEX IF NOT EXISTS idx_smart_knowledge_access ON public.smart_knowledge_items(access_level);
CREATE INDEX IF NOT EXISTS idx_smart_knowledge_created_by ON public.smart_knowledge_items(created_by);
