-- Fix prior failure: ensure unified_ai_logs is a VIEW by dropping conflicting table if present, then re-apply everything idempotently

-- 0) Ensure unified_ai_logs is not an existing TABLE
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema = 'public' AND table_name = 'unified_ai_logs'
  ) THEN
    EXECUTE 'DROP TABLE public.unified_ai_logs';
  END IF;
END $$;

-- Re-apply event_bus, functions, triggers, indexes, and view
