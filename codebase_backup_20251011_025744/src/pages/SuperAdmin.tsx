...
import { GoogleCloudConfigStatus } from '@/components/admin/GoogleCloudConfigStatus';
...
export default function SuperAdminPage() {
  ...
  const isSuperadminByMetadata = user?.user_metadata?.role === 'global_superadmin' || 
  ...
  const isSuperadminByUserId = user?.id === '339e3acc-a5a0-43ff-ae07-924fc32a292a' ||
                               user?.id === '6d201176-5be1-45d4-b09f-f70cb4ad38ac' ||
  ...
  // If authenticated but not superadmin, show detailed access denied info
  if (!effectiveIsSuper) {
    return (
      <div className="container mx-auto p-6 space-y-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Access Denied - Superadmin Required</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-muted-foreground">
              You must be a SuperAdmin to access the Developer Configuration. 
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-2">
              ...
              <div className="text-sm space-y-1">
                <div>User ID: <code className="bg-gray-200 px-1 rounded">{user?.id}</code></div>
                <div>Email Match: <code className="bg-gray-200 px-1 rounded">{isSuperadminByEmail ? '✅ Yes' : '❌ No'}</code></div>
                <div>Metadata Check: <code className="bg-gray-200 px-1 rounded">{isSuperadminByMetadata ? '✅ Yes' : '❌ No'}</code></div>
                <div>ID Match: <code className="bg-gray-200 px-1 rounded">{isSuperadminByUserId ? '✅ Yes' : '❌ No'}</code></div>
                <div>Hook Result: <code className="bg-gray-200 px-1 rounded">{isSuper ? '✅ Yes' : '❌ No'}</code></div>
                <div>Final Access: <code className="bg-gray-200 px-1 rounded">{effectiveIsSuper ? '✅ Granted' : '❌ Denied'}</code></div>
              </div>
            </div>
