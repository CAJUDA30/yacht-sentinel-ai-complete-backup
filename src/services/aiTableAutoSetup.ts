...
export class AITableAutoSetup {
  ...
  static async checkTableSchema(): Promise<{ hasCorrectSchema: boolean; hasConfigColumn: boolean }> {
    try {
      // Try to insert a test record to check if config column exists
      const testData = {
  ...
  static async createProvidersTable(): Promise<SetupResult> {
    ...
    try {
      ...
      if (insertError) {
        console.error('❌ Provider insertion failed:', insertError);
  ...
  static async ensureSetup(): Promise<SetupResult> {
    ...
    if (tableExists) {
      console.log('✅ ai_providers_unified table exists, checking schema...');
      ...
      if (hasCorrectSchema) {
        ...
        console.log('⚠️ Table exists but has incorrect schema, attempting to fix...');
        
        if (!hasConfigColumn) {
          // Attempt to migrate the existing table by using the configuration column instead
          console.log('🔄 Attempting schema migration for local database...');
          ...
          try {
            ...
            if (!readError && existingData && existingData.length > 0) {
              console.log(`📋 Found ${existingData.length} existing providers, migrating to new schema...`);
            ...
            console.warn('Migration failed, proceeding with table creation...');
          }
        }
    ...
    console.log('⚠️ ai_providers_unified table missing, creating...');
    return await this.createProvidersTable();
  }
