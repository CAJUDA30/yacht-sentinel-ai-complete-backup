...
class SystemHealthService {
  ...
  private async checkProcessorsHealth(): Promise<ProcessorHealthSummary> {
    try {
      ...
      // Call the GCP unified config edge function to test document processors
      const { data, error } = await supabase.functions.invoke('gcp-unified-config', {
        body: { action: 'test_all_connections' }
      });

      if (error) {
        debugConsole.error('SYSTEM_HEALTH', 'Processor health check failed', { error: error.message });
        return {
          total: 1,
          healthy: 0,
          unhealthy: 1,
