...
export function useStartupHealthCheck(options: UseStartupHealthCheckOptions = {}) {
  ...
  useEffect(() => {
    ...
    if (runOnAppStart && !hasRunOnAppStart.current && !isHealthCheckInitialized.current) {
      ...
      const timeoutId = setTimeout(async () => {
        try {
          ...
          startupHealthService.schedulePeriodicChecks(30);
  ...
  useEffect(() => {
    if (!enabled || loading || aiInitializing || !aiInitialized || !runOnLogin || !isHealthCheckInitialized.current) return;
